...
- [x] 旧后台文章列表总数 = 新后台文章列表总数
- [x] 旧后台标签列表 = 新后台标签列表（顺序可能不同，内容一致）
- [x] 旧后台分类列表 = 新后台分类列表
- [x] 旧后台友链列表 = 新后台友链列表（含 sortOrder）
...
- [x] 通过新后台完成一篇文章的完整生命周期（创建→保存→发布→修改→下架→删除）
- [x] 创建一个新用户，分配"内容编辑"角色，登录后只能看到文章/标签/分类菜单
- [x] 创建一个新角色，赋予部分权限，分配给用户验证按钮级权限
- [x] 上传 5 张图片，在媒体库可见，可复制链接到文章
- [x] 旧 EJS 后台 `/admin/posts` 与新 `/posts` 数据完全一致
- [x] 数据导出 → 清空数据库 → 导入还原，数据无损
- [x] 所有写操作在 `audit_logs` 表能查到记录
