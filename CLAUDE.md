...
- **Front port (8787)**: Express 5 + SQLite legacy blog (static HTML frontend).
- **Admin port (3000)**: Express 5 + JWT + RBAC v2 API, consumed by a Vue 3 SPA in `admin/`.

Phase 1 (completed) established the dual-port foundation, JWT auth, RBAC seed, Vue 3 admin skeleton, Docker multi-stage build, and Unraid deployment.
...
docker run -d \
  ...
  -v /mnt/user/appdata/blog-design/db:/app/server/db \
  -v /mnt/user/appdata/blog-design/uploads:/app/server/public/uploads \
  ghcr.io/ifoxchen-glitch/blog-design-v2.0:latest
```

**Container runs as non-root user `nodejs` (UID 1001).** The mounted host directories must be writable by UID 1001:
```bash
chown -R 1001:1001 /mnt/user/appdata/blog-design/db
chown -R 1001:1001 /mnt/user/appdata/blog-design/uploads
```
...
- `frontApp` @ `PORT` (default 8787) — legacy blog (static HTML).
- `adminApp` @ `ADMIN_PORT` (default 3000) — v2 JWT/RBAC API.

**Key files**:
- `server/src/apps/frontApp.js` — Public blog API + static file serving. All legacy EJS admin routes removed (return 410 Gone).
...
- `server/src/auth.js` — Session-based auth (legacy, no longer used).
...
- `server/src/utils/backup.js` — SQLite backup via `VACUUM INTO` (Alpine compatible).
- `server/src/jobs/backupSqlite.js` — Daily cron backup at 02:00.
- `server/src/jobs/cleanupAuditLogs.js` — Daily cron cleanup at 03:00 (respects `AUDIT_LOG_RETENTION_DAYS`).
...
- `js/theme.js` — 11 themes + dark/light mode with `localStorage` persistence.
...
- `admin/src/components/layout/AdminLayout.vue` — Collapsible sidebar with ionicons5 icons, top bar (logo, breadcrumb, user dropdown), main `<RouterView>`. Dark theme (#0f172a sidebar).
- `admin/src/views/dashboard/index.vue` — Dashboard with stat cards, charts (Line, Bar, Pie), CSV export.
...
- All `/admin/*` routes and `/api/admin/*` endpoints return **410 Gone**.
- Old EJS admin pages and session-based auth removed.
...
- `POST /api/v2/backup` — SQLite backup via `VACUUM INTO`.
...
**Port mappings**:
- `8787` → Front blog
- `3000` → Admin v2 SPA/API

**Volume mappings**:
- `/mnt/user/appdata/blog-design/db` → `/app/server/db`
- `/mnt/user/appdata/blog-design/uploads` → `/app/server/public/uploads`

**Environment variables** (set in Unraid Docker UI):
- `JWT_SECRET` — Strong random string (≥32 chars). Auto-generated fallback if unset (not recommended for production).
...
docker run -d \
  ...
  --cleanup --interval 3600 Blog-Design
```
...
- Express 5 path-to-regexp v8: bare `*` wildcard not supported. Use `app.use("/path", handler)` instead of `app.get("/path/*")`.
- Container runs as `nodejs` user (UID 1001). Host mount directories must be `chown -R 1001:1001`.
- `.dockerignore` excludes all `.env.*` files and `server/db/*.sqlite*` to prevent secrets and DB from being baked into the image.
