...
const pagedTags = computed(() => {
  ...
  return sortedTags.value.slice(start, start + cardPageSize.value)
})
...
<template>
  <div>
    ...
    <template v-if="viewMode === 'card'">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        <div
          ...
          :class="[
            ...
            selectedSlug === row.slug ? getColor((cardPage - 1) * cardPageSize + idx).border : '',
          ]"
      ...
      <!-- 卡片分页 -->
      <div v-if="sortedTags.length > 0" class="mt-5 flex justify-center">
        <NPagination
          v-model:page="cardPage"
          v-model:page-size="cardPageSize"
          :item-count="sortedTags.length"
